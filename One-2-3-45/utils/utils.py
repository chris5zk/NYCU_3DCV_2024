...
def get_poses(init_elev):
    ...
    if init_elev <= 75:
        ...
        elevations = np.radians([mid]*8 + [low]*8 + [mid-deg,mid+deg,mid,mid]*8 + [low-deg,low+deg,low,low]*8)
        img_ids = [f"{num}.png" for num in range(16)] + [f"{num}_{view_num}.png" for num in range(16) for view_num in range(4)]
    else:
        high = init_elev - 30
        elevations = np.radians([mid]*8 + [high]*8 + [mid-deg,mid+deg,mid,mid]*8 + [high-deg,high+deg,high,high]*8)
        img_ids = [f"{num}.png" for num in list(range(8)) + list(range(16,24))]  + \
                [f"{num}_{view_num}.png" for num in list(range(8)) + list(range(16,24)) for view_num in range(4)]
    overlook_theta = [30+x*90 for x in range(8)]
    eyelevel_theta = [60+x*90 for x in range(8)]
