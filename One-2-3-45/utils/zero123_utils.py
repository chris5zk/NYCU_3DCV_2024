...
def predict_stage1_gradio(model, raw_im, save_path = "", adjust_set=[], device="cuda", ddim_steps=75, scale=3.0):
    ...
    # stage 1: 8 -> 16
    delta_x_1_8 = [0] * 8 + [30] * 8 + [-30] * 8
    delta_y_1_8 = [0+45*(i%8) if i < 8 else 15+45*(i%8) for i in range(16)] + [30+45*(i%8) for i in range(24)]
    ...
    if adjust_set != []:
        ...
        x_samples_ddims_8 = sample_model_batch(model, sampler, input_im, delta_x_1_8, delta_y_1_8, 
                                               n_samples=len(delta_x_1_8), ddim_steps=ddim_steps, scale=scale)
    sample_idx = 0
...
def zero123_infer(model, input_dir_path, start_idx=0, end_idx=12, indices=None, device="cuda", ddim_steps=75, scale=3.0):
    ...
    infer_stage_2(model, save_path_8, save_path_8_2, delta_x_2, delta_y_2, 
                  indices=indices if indices else list(range(start_idx,end_idx)), device=device, ddim_steps=ddim_steps, scale=scale)
