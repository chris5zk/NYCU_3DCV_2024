...
def stage1_run(model, device, exp_dir,
    ...
    if polar_angle <= 75:
        output_ims_2 = predict_stage1_gradio(model, input_im, save_path=stage1_dir, adjust_set=list(range(8,16)), device=device, ddim_steps=ddim_steps, scale=scale)
    else:
        output_ims_2 = predict_stage1_gradio(model, input_im, save_path=stage1_dir, adjust_set=list(range(16,24)), device=device, ddim_steps=ddim_steps, scale=scale)
    torch.cuda.empty_cache()
...
def stage2_run(model, device, exp_dir, elev, scale, stage2_steps=50):
    ...
    if 90-elev <= 75:
        zero123_infer(model, exp_dir, indices=list(range(1,16)), device=device, ddim_steps=stage2_steps, scale=scale)
    else:
        zero123_infer(model, exp_dir, indices=list(range(1,8))+list(range(16,24)), device=device, ddim_steps=stage2_steps, scale=scale)
